...
function IconPrev()     { return <svg width="12" height="12" viewBox="0 0 16 16" fill="currentColor"><path d="M3 2h2v12H3zm10 0L7 8l6 6V2z"/></svg>; }
function IconNext()     { return <svg width="12" height="12" viewBox="0 0 16 16" fill="currentColor"><path d="M11 2h2v12h-2zM3 2l6 6-6 6V2z"/></svg>; }
function IconRew()      { return <svg width="12" height="12" viewBox="0 0 16 16" fill="currentColor"><path d="M8 8L2 4v8zm6 0L8 4v8z"/></svg>; }
function IconFwd()      { return <svg width="12" height="12" viewBox="0 0 16 16" fill="currentColor"><path d="M8 8l6-4v8zm-6 0l6-4v8z"/></svg>; }
function IconPlay()     { return <svg width="12" height="12" viewBox="0 0 16 16" fill="currentColor"><path d="M4 2l10 6-10 6V2z"/></svg>; }
function IconPause()    { return <svg width="12" height="12" viewBox="0 0 16 16" fill="currentColor"><rect x="3" y="2" width="4" height="12"/><rect x="9" y="2" width="4" height="12"/></svg>; }
function IconMusic()    { return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>; }

function CtrlBtn({ onClick, children, label }: { onClick: () => void; children: React.ReactNode; label: string }) {
  return (
    <button
      onClick={onClick}
      title={label}
...
function Marquee({ text, className }: { text: string; className?: string }) {
  ...
  useEffect(() => {
    ...
    span.style.animationDuration = overflow ? `${Math.max(6, span.scrollWidth / 40)}s` : "0s";
  });
    ...
    >
      ...
      {nowPlaying && (
        <div className="flex justify-between" style={{ marginTop: -2 }}>
          <span className="text-[8px] tabular-nums text-white/20">{formatTime(nowPlaying.position)}</span>
          <span className="text-[8px] tabular-nums text-white/15">{formatTime(nowPlaying.duration)}</span>
        </div>
      )}

      {/* Controls */}
      <div className="flex items-center justify-between">
        <CtrlBtn onClick={previousTrack} label="Previous track">
          <IconPrev />
        </CtrlBtn>
        <CtrlBtn onClick={skipBackward} label="Back 15s">
          <IconRew />
        </CtrlBtn>
        <button
          onClick={togglePlay}
        ...
        <CtrlBtn onClick={skipForward} label="Forward 15s">
          <IconFwd />
        </CtrlBtn>
        <CtrlBtn onClick={nextTrack} label="Next track">
          <IconNext />
        </CtrlBtn>
      </div>
    </GlassTile>
